    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if input_by_ph.count() > 0 and output_by_ph.count() > 0 and input_by_ph.is_visible() and output_by_ph.is_visible():
                action = page.locator("button").filter(has_text=re.compile(r"Transliterat", re.IGNORECASE)).first
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                # Click the website button and wait until Sinhala output is available.
                # This version does NOT fail just because the output is the same as the previous row.
                if action_locator:
                    try:
                        action_locator.wait_for(state="visible", timeout=max(1000, int(args.timeout_ms)))
                        action_locator.click(timeout=max(1000, int(args.timeout_ms)))
                    except Exception:
                        # Fallback: click the first visible button containing Transliterat.
                        page.locator("button").filter(has_text=re.compile(r"Transliterat", re.IGNORECASE)).first.click(timeout=max(1000, int(args.timeout_ms)))

                actual_output = ""
                deadline = time.time() + max(1, int(args.wait_ms)) / 1000
                while time.time() < deadline:
                ...
                # If the site is still slow, do a few quick retries without changing columns.
                ...
                for _ in range(tries):
                    ...
                    if current:
                        actual_output = current
                        break
                    page.wait_for_timeout(max(100, int(args.retry_wait_ms)))
